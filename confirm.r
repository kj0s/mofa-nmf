...
FATE_RDS <- "/vast/projects/Sisseq/human-haematopoiesis-sis-seq/data/ST223.annotated.rds"
...
barcode_rep_summary <- sc@meta.data %>%
  group_by(barcode15) %>%
  summarise(n_cells = n(), .groups = "drop") %>%
  arrange(desc(n_cells))

print(barcode_rep_summary)

# Filter for barcodes present end-to-end
present_barcodes <- report %>%
  filter(status == "present_end_to_end") %>%
  select(
    barcode_core, 
    n_cells_in_raw_pool, 
  ...
  arrange(desc(n_cells_in_fate_rds))

# Print the full list of overlapping barcodes
print(present_barcodes, n = Inf)

# Quick summary stats on the overlapping clones
cat(sprintf("\nTotal unique barcodes in both files: %d\n", nrow(present_barcodes)))
cat(sprintf("Average cells per barcode in raw pool: %.1f\n", mean(present_barcodes$n_cells_in_raw_pool)))
cat(sprintf("Average cells per barcode in final RDS: %.1f\n", mean(present_barcodes$n_cells_in_fate_rds)))
